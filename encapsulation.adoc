...
Two types of encapsulation are defined: _normal_ and _null_.

=== Normal Encapsulation Structure

The normal encapsulation structure is comprised of four field-groups as shown in <<_table_Groups>>:
...
Timestamps may be omitted either because time is not of interest to the user, or if time information is already included within the encapsulated payload (for example some E-Trace packets include may optionally include a *time* field).
...
* For two _normal_ packets with M _nulls_ between them, this would comprise M-1 _null.idles_ and 1 _null.alignment_
* The synchronization sequence will comprise N _null.idles_ followed by 1 _null.alignment_
